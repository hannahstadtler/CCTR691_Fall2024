...
> Red Flag 1: There are only 10,349 reads per cell in this sample; we are really hoping for 20-25K at least in order to do differential expression analysis. 

> Red Flag 2: Our expected number of cells for this sample was 5000 but our sample actually had 17,000+ cells
...
setwd("~/CCTR691/24.09.26_BnfoLab")
```{r}
library(Seurat)
library(dplyr)
```
...
> There are 2,575 samples within 1 assay, which refers to the number of cells. The 19,612 number refers to the amount of RNA that is amplified in the sample. 
...
> Last class we talked about cutting off samples at 25%, so anything with less than 25% expression can be included. 
...
> There are 1,314 cells that survived the filtering. 
...
> After filtering, there are a lot fewer cells that have close to 0 nFeature (meaning the number of genes expressed are closer to 0), which is similar for nCount_RNA as well. It seems like a couple of the extreme outliers on nCount_RNA were excluded as well. 
...
> Each heatmap is a different principle component analysis, and different genes (out of the 19,000+ in our reference genome) are on the Y-axis. We have 15 different PCAs from this analysis, per our code, the first starting with the cells and genes that are the most variable (and thus interesting for our analysis), and it gives us 15 different options ranking from most variable to less variable. 

Q10) How many cells are used in each heatmap?  

> 500, because that's what we told it to do in our code. We can include as many or as few as we'd like. 

Q11) Describe what happens when the principal component (PC) number gets higher.

> The variability becomes less vs. earlier PC numbers that have pulled more varability out. 
...
> It shifts the skew and output of your data
...
> It becomes lines instead of individual dots/a broader spectrum of data points 

```{r}
whim_filtered <- RunTSNE(whim_filtered, dims = 1:100)
DimPlot(whim_filtered, reduction = "tsne")
```

What happens when you use more dimensions?

> There is more of a spectrum for the data points
...
> The different clusters that we identified in one of the above questions 
